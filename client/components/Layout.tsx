...
export default function Layout({ children }: LayoutProps) {
  ...
  const {
    ...
    toolNotifications,
    pendingAutomationResults,
    resolveAutomationResult,
  } = useNotifications();

  const activeAutomationResults = useMemo(
    () => pendingAutomationResults.filter((result) => !result.resolved),
    [pendingAutomationResults],
  );
                  ...
                  {toolsOpen && (
                    <div className="absolute right-0 mt-2 w-72 bg-white border rounded-md shadow-lg p-2 z-50">
                      ...
                      <div className="mt-3 border-t border-slate-100 pt-3">
                        ...
                        {activeAutomationResults.length === 0 ? (
                          <p className="px-3 py-2 text-xs text-slate-500">
                            No pending automation tasks.
                          </p>
                        ) : (
                          <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                            {activeAutomationResults.map((result) => (
                              <div
                                key={result.id}
                                ...
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    ...
                                    <p className="mt-2 text-[10px] uppercase tracking-wide text-slate-400">
                                      {automationDateFormatter.format(new Date(result.createdAt))}
                                    </p>
                                    {result.href ? (
                                      <Link
                                        to={result.href}
                                        className="mt-2 inline-flex items-center text-[11px] font-medium text-blue-600 hover:text-blue-700"
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => resolveAutomationResult(result.id)}
            ...
            <nav className="max-w-7xl mx-auto px-4 py-4 space-y-1">
              ...
              <div className="mt-2 border-t pt-2">
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {activeAutomationResults.length === 0 ? (
                    <p className="px-4 text-xs text-slate-500">All caught up on automation runs.</p>
                  ) : (
                    activeAutomationResults.map((result) => (
                      <div
                        key={result.id}
                        className="mx-4 rounded-lg border border-slate-200 bg-white px-3 py-2"
                      >
